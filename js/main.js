...
async function getAllFoodItems() {
    ...
    for (let item of allFoodItems.foods) {
        ...
        newLink.addEventListener('mouseover', async function () {
            if (newDiv.lastChild.tagName !== 'TABLE') {
                const foodItemValues = await getData(
                    `${baseUrl}login/plan/values?food_id=${item[1]}`)
                ...
                newTd.innerHTML = foodItemValues['carbs'];
                newTr.appendChild(newTd);
                newTd = document.createElement('td');
                newTd.innerHTML = foodItemValues['fat'];
                newTr.appendChild(newTd);
                newTd = document.createElement('td');
                newTd.innerHTML = foodItemValues['protein'];
                newTr.appendChild(newTd);
                newTd = document.createElement('td');
                newTd.innerHTML = foodItemValues['energy'];
                newTr.appendChild(newTd);
                newTable.classList.add('food-values');
                newTable.appendChild(newTr);
                newDiv.appendChild(newTable);
                newLink.addEventListener('mouseover', (evt) => {
